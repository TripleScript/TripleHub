...
if not isfolder("triple hub") then
    makefolder("triple hub")
end
...
function Library:save_flags()
    ...
    writefile(`triple hub/{game.GameId}.lua`, flags)
end

function Library:load_flags()
    if not isfile(`triple hub/{game.GameId}.lua`) then 
Library.save_flags() 
  return
 end

    local flags = readfile(`triple hub/{game.GameId}.lua`)
    if not flags then Library.save_flags()
 return 
end
